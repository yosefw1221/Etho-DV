  ...
  try {
    ...
    const enrichedPayments = await Promise.all(
      pendingPayments.map(async (payment) => {
        // Get user details
        const user = await User.findById(payment.user_id).select('first_name last_name email phone role business_name');
        
        // Get form details
        const form = await Form.findById(payment.form_id).select('applicant_data processing_status reference_number');
...
export const GET = requireRole(['admin', 'operator'])(getPendingPaymentsHandler);