...
export function validatePersonalInfo(data: DVFormData): FormValidation {
    ...
    if (isNaN(birthDate.getTime())) {
      ...
      if (birthDate > today) {
        ...
        if (age < 18) {
          errors.push({
            field: 'date_of_birth',
            message: 'Must be at least 18 years old to apply for DV lottery',
          });
        }
  ...
  if (!personal_info.place_of_birth?.trim()) {
    ...
    errors.push({
      field: 'place_of_birth',
      message: 'Place of birth must be at least 2 characters long',
    });
  } else if (personal_info.place_of_birth.trim().length > 100) {
    errors.push({
      field: 'place_of_birth',
      message: 'Place of birth cannot exceed 100 characters',
    });
  }
  ...
  if (
    personal_info.country_of_eligibility &&
    personal_info.country_of_eligibility.trim()
  ) {
    if (personal_info.country_of_eligibility.trim().length < 2) {
      errors.push({
        field: 'country_of_eligibility',
        message: 'Please enter a valid country name',
      });
    }
  }
...
export function validateContactInfo(data: DVFormData): FormValidation {
  ...
  if (!contact_info.phone?.trim()) {
    ...
    if (
      !ethiopianPhoneRegex.test(phone.replace(/\s/g, '')) &&
      !internationalPhoneRegex.test(phone)
    ) {
      errors.push({
        field: 'phone',
        message:
          'Please enter a valid phone number (e.g., +251912345678 or 0912345678)',
      });
    }
  }
    ...
    if (isNaN(expiryDate.getTime())) {
      ...
      // Check if passport expires within 6 months (warning)
        ...
        errors.push({
          field: 'passport_expiry',
          message:
            'Passport expires within 6 months. Consider renewing before travel.',
        });
      }
...
export function validateBackgroundInfo(data: DVFormData): FormValidation {
  ...
  if (background_info.occupation && background_info.occupation.trim()) {
    if (background_info.occupation.trim().length < 2) {
      ...
      !/^[a-zA-Z\s\-'\.&,]+$/.test(background_info.occupation.trim())
    ) {
      errors.push({
        field: 'occupation',
        message:
          'Occupation can only contain letters, spaces, hyphens, apostrophes, periods, ampersands, and commas',
      });
    }
  }
...
export function validateFamilyMember(member: any): ValidationError[] {
      ...
      if (birthDate > today) {
        ...
        if (member.relationship_type === 'child' && age >= 21) {
          errors.push({
            field: `${prefix}_date_of_birth`,
            message:
              'Children must be under 21 years old to be included in DV application',
          });
        }
  ...
  if (!member.country_of_birth?.trim()) {
    errors.push({
      field: `${prefix}_country_of_birth`,
      message: 'Country of birth is required',
    });
  ...
  if (!member.photo) {
    errors.push({
      field: `${prefix}_photo`,
      message: 'Photo is required for all family members',
    });
  }
...
export function validateFamilyInfo(data: DVFormData): FormValidation {
  ...
  if (background_info.marital_status === 'Married') {
    const hasSpouse = family_members.some(
  ...
  const spouseCount = family_members.filter(
    (member) => member.relationship_type === 'spouse'
  ).length;
  if (spouseCount > 1) {
    errors.push({
...
export function validatePhotoUpload(data: DVFormData): FormValidation {
  ...
  if (!photo) {
    errors.push({
      field: 'photo',
      message: 'Photo is required for the primary applicant',
    });
  } else {
    ...
    if (typeof photo === 'object' && 'url' in photo) {
      ...
      } else if (photo.size && photo.size > 240000) {
        // 240KB limit
        errors.push({
          field: 'photo',
          message: 'Photo size must be less than 240KB',
        });
      }
    }
    // Check if photo is File type
      ...
      if (photo.size === 0) {
        errors.push({
          field: 'photo',
          message: 'Photo file is empty. Please select a valid photo.',
        });
      } else if (photo.size > 240000) {
        // 240KB limit
        errors.push({
          field: 'photo',
          message: 'Photo size must be less than 240KB',
...
export function validateBusinessRules(data: DVFormData): FormValidation {
  ...
  if (background_info.education_level) {
    ...
    if (lowEducationLevels.includes(background_info.education_level)) {
      ...
      if (
        !background_info.occupation ||
        background_info.occupation.trim() === ''
      ) {
        errors.push({
          field: 'occupation',
          message:
            'Work experience is required for applicants with primary or incomplete high school education',
        });
      }
    }
  }
  ...
  const children = family_members.filter(
    ...
    if (child.date_of_birth) {
      ...
      if (age >= 21) {
        errors.push({
          ...
          message: `Child ${
            index + 1
          } is 21 or older and cannot be included in DV application`,
        });
      }
    }
  });
  ...
  const spouses = family_members.filter(
    (member) => member.relationship_type === 'spouse'
  );
  if (spouses.length > 1) {
    errors.push({
  ...
  if (background_info.marital_status === 'Single' && spouses.length > 0) {
    errors.push({
      field: 'family_members',
      message:
        'Spouse information should not be provided for single applicants',
    });
  }
  ...
  if (
    ...
    !eligibleCountries.includes(personal_info.country_of_birth)
  ) {
    errors.push({
      field: 'country_of_birth',
  ...
  if (duplicateNames.length > 0) {
    errors.push({
      field: 'family_members',
      message:
        'Duplicate names found. Each family member must have a unique name.',
    });
  }
  ...
  switch (fieldName) {
    ...
    case 'last_name':
      ...
      if (trimmedValue.length > 50) {
        return {
          field: fieldName,
          message: `${fieldName.replace('_', ' ')} cannot exceed 50 characters`,
        };
      }
    ...
    case 'phone':
      ...
      const internationalPhoneRegex = /^\+[1-9]\d{1,14}$/;
      if (
        !ethiopianPhoneRegex.test(trimmedValue.replace(/\s/g, '')) &&
        !internationalPhoneRegex.test(trimmedValue)
      ) {
        return {
          field: fieldName,
          message:
